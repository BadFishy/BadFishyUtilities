...
#define V_MSG(Param1) (GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, *(JOYSTR_CUR_CLASS_LINE + ": " + Param1)) )

#define V_MSG2(Param1,Param2) (GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, *(JOYSTR_CUR_CLASS_LINE + ": " + Param1 + " " + Param2)) )

#define V_MSGF(Param1,Param2) (GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, *(JOYSTR_CUR_CLASS_LINE + ": " + Param1 + " " + FString::SanitizeFloat(Param2))) )
...
#define BF_LOG(LogCat, LogMode, FormatString , ...) BF_LOGS(this, LogCat, LogMode, FormatString , ##__VA_ARGS__)

#define BF_LOGS(Sender, LogCat, LogMode, FormatString , ...) \
	{ \
	FString Message = FString::Printf(TEXT("%s: %s"), *JOYSTR_CUR_CLASS_LINE, *FString::Printf(TEXT(FormatString), ##__VA_ARGS__)); \
	if(Sender != nullptr) \
		{ \
			Message +=  FString::Printf(TEXT(" (%s)"), *Sender->GetName() ); \
		} \
	UE_LOG(LogCat, LogMode, TEXT("%s"), *Message); \
	}
